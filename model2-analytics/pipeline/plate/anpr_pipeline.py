...
from pipeline.ocr.ocr_engine import EasyOCREngine  # Fallback
...
class ANPRPipeline(PlateRecognizerInterface):
    ...
    def recognize(
        ...
        h, w = frame.shape[:2]
        vh = y2 - y1  # vehicle height
        vw = x2 - x1  # vehicle width

        # Define multiple search regions (with margins)
        margin_x = int(vw * 0.20)
        margin_y_bot = int(vh * 0.30)
        
        regions = []
        
        # Region 1: Bottom 60% (rear plates - most common)
        regions.append({
            'name': 'bottom_60',
            'y1': y1 + int(vh * 0.40),
            'y2': min(h, y2 + margin_y_bot),
            'x1': max(0, x1 - margin_x),
            'x2': min(w, x2 + margin_x),
        })
        
        # Region 2: Bottom 40% (compact cars, lower plates)
        regions.append({
            'name': 'bottom_40',
            'y1': y1 + int(vh * 0.60),
            'y2': min(h, y2 + margin_y_bot),
        ...
        # Region 3: Full vehicle bbox (side/front views)
        regions.append({
            'name': 'full_bbox',
            'y1': y1,
            'y2': y2,
        ...
        # Try each region and collect all valid plate detections
        all_results = []
