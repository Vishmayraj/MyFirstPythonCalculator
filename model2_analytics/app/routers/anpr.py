...
async def process_image(file: UploadFile = File(...), expected_plate: Optional[str] = Form(None)):
    ...
    try:
        ...
        for det in plate_dets:
            ...
            if crop.size == 0:
                continue
            result = ocr_engine.read_plate(crop)
            if result and result.plate_text:
                if best is None or result.confidence > best["confidence"]:
                    best = {"text": result.plate_text, "confidence": result.confidence, "bbox": det.bbox, "det_conf": det.confidence}

        steps["4"] = {"status": "success" if best else "error", "time": f"{time.time()-t0:.2f}s", "detail": best["text"] if best else "No plate read"}

        # Save crop image
        crop_path = None
        if best:
            crop_dir = Path(__file__).resolve().parent.parent.parent / "model2_analytics" / "detection-image"
        ...
        if db:
            try:
                ...
                normalized = normalize_plate(best["text"]) if best else ""
        ...
        response = {
            "success": True,
            "steps": steps,
            "plate": {"text": best["text"], "normalized": normalized, "confidence": best["confidence"], "detection_confidence": best["det_conf"], "bbox": [int(c) for c in best["bbox"]]} if best else None,
